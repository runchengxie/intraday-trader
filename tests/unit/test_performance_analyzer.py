...
def populated_analyzer():
    ...
    mock_trades = [
        TradeRecord(timestamp=datetime(2023, 1, 5, 10, 5), symbol='SPY', side='buy', quantity=50, price=380.00, commission=1.0, order_id='order1', slippage=0.02, market_impact=0.01),
        TradeRecord(timestamp=datetime(2023, 1, 6, 14, 30), symbol='SPY', side='sell', quantity=50, price=385.00, commission=1.0, order_id='order2', slippage=0.03, market_impact=0.015), # Profit
        TradeRecord(timestamp=datetime(2023, 1, 10, 11, 0), symbol='AAPL', side='buy', quantity=25, price=130.00, commission=1.0, order_id='order3', slippage=0.01, market_impact=0.005),
        TradeRecord(timestamp=datetime(2023, 1, 11, 9, 45), symbol='AAPL', side='sell', quantity=25, price=128.00, commission=1.0, order_id='order4', slippage=0.01, market_impact=0.005), # Loss
    ]
...
def test_add_trade(empty_analyzer):
    ...
    trade = TradeRecord(timestamp=datetime.now(), symbol='SPY', side='buy', quantity=10, price=400.0, commission=1.0, order_id='t1')
...
def test_calculate_turnover_rate(populated_analyzer):
    ...
    avg_portfolio_value = np.mean([100000.0, 100150.0, 100247.0, 100200.0, 100100.0, 100193.0])
...
def test_plot_performance_charts_runs_without_error(populated_analyzer, tmp_path, mocker):
    """
